...
SELENIUM="$(ls ${DOWNLOADS}/SeleniumGridExtras*.jar | head -n 1)"
...
configure() {
	...
	VBoxManage guestcontrol $id execute --image "C:/Temp/vagrant_prepare.bat" ${GC_OPTS} --wait-exit --wait-stdout --wait-stderr || exit 1
...
install_selenium() {
	...
	VBoxManage guestcontrol $id copyto "${SELENIUM}" 'C:/selenium/selenium-grid-extras.jar' ${GC_OPTS} || exit 1
	...
	VBoxManage guestcontrol $id copyto "${FILES}/selenium_conf/selenium_grid_extras_config.json.example" 'C:/Windows/System32/selenium_grid_extras_config.json.example' ${GC_OPTS} || exit 1
	VBoxManage guestcontrol $id copyto "${FILES}/selenium_conf/selenium_grid_extras_config.json" 'C:/Windows/System32/selenium_grid_extras_config.json' ${GC_OPTS} || exit 1
	VBoxManage guestcontrol $id copyto "${FILES}/selenium_conf/WIN7/${VM_IE}/node_5555.json" 'C:/Windows/System32/node_5555.json' ${GC_OPTS} || exit 1
